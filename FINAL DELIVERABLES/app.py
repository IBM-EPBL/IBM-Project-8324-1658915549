...
from flask import Flask, render_tempate, request, url_for
import os
#import sendgrid
...
dsn = (
    ...
    "UID={5};"
    "PWD={6};"
    "SECURITY={7};").format(dsn_driver, dsn_database, dsn_hostna, dsn_port, dsn_protocol, dsn_uid, dsn_pwd,dsn_security)
...
def jobseeker():
    if request.method == 'POST':
        ...
        try:
            ...
            #sql = "insert into REGISTER_AS_REGISTER values ('{}','{}','{}','{}','{}')".format( name, email, password, ages, skills, expectedjob)
...
def applyforjob():
    if request.method == 'POST':
        ...
        email = request.form.get('email')
        #message = Mail(from_email=email,to_emails='mail2me@gmail.com',subject='Resume',html_content=resume)
        
        #response = sg.send(message)
        #return response
       
        from_email=Email("mail2me@gmail.com")
        to_email=Email("mail2me@gmail.com")
        subject="mail"
        content=Content("text/plain","easy")
        mail=Mail(from_email,subject,to_email,content)
        response=sg.client.mail.send.pst(request_body=mail.get())'''
    return render_template('upload_resume.html')
@app.route('/manualrecommendation',methods=['POST','GET'])
def manualrecommedation():
    if request.method == 'POST':
